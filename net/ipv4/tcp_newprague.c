...
/* TCP newPrague congestion control.
 *
 ...
 *
 * 1/ RTT independence:
 * newprague will operate in a given RTT region as if it was experiencing a target
 ...
 *
 * 5/ Pacing/tso sizing
 * newprague aims to keep queuing delay as low as possible. To that end, it is in
...
#define pr_fmt(fmt) "TCP-newPrague " fmt
...
#define NEWPRAGUE_ALPHA_BITS	24U
#define NEWPRAGUE_MAX_ALPHA	(1ULL << NEWPRAGUE_ALPHA_BITS)
#define CWND_UNIT		20U
#define ONE_CWND		(1ULL << CWND_UNIT)
#define NEWPRAGUE_SHIFT_G	4		/* EWMA gain g = 1/2^4 */
...
static u32 newprague_burst_shift __read_mostly = 12; /* 1/2^12 sec ~=.25ms */
MODULE_PARM_DESC(newprague_burst_shift,
		 "maximal GSO burst duration as a base-2 negative exponent");
module_param(newprague_burst_shift, uint, 0644);

static u32 newprague_max_tso_segs __read_mostly = 0;
MODULE_PARM_DESC(newprague_max_tso_segs, "Maximum TSO/GSO segments");
module_param(newprague_max_tso_segs, uint, 0644);

static u32 newprague_rtt_target __read_mostly = 25 * USEC_PER_MSEC;
MODULE_PARM_DESC(newprague_rtt_target, "RTT scaling target");
module_param(newprague_rtt_target, uint, 0644);

static int newprague_rtt_transition __read_mostly = DEFAULT_RTT_TRANSITION;
MODULE_PARM_DESC(newprague_rtt_transition, "Amount of post-SS rounds to transition"
		 " to be RTT independent.");
module_param(newprague_rtt_transition, uint, 0644);

static int newprague_rate_offset __read_mostly = RATE_OFFSET; /* 4/128 ~= 3% */
MODULE_PARM_DESC(newprague_rate_offset,
		 "Pacing rate offset in 1/128 units at each half of RTT_virt");
module_param(newprague_rate_offset, uint, 0644);
...
static struct tcp_congestion_ops newprague_reno;

static void __newprague_connection_id(struct sock *sk, char *str, size_t len)
{
...
#define LOG(sk, fmt, ...) do {						\
	...
	__newprague_connection_id(sk, __tmp, sizeof(__tmp));		\
	/* pr_fmt expects the connection ID*/				\
	pr_info("(%s) : " fmt "\n", __tmp, ##__VA_ARGS__);		\
} while (0)

static struct newprague *newprague_ca(struct sock *sk)
{
	return (struct newprague*)inet_csk_ca(sk);
}

static bool newprague_is_rtt_indep(struct sock *sk)
{
	struct newprague *ca = newprague_ca(sk);

	return !tcp_in_slow_start(tcp_sk(sk)) &&
		ca->round >= ca->rtt_transition_delay;
}

static bool newprague_e2e_rtt_elapsed(struct sock *sk)
{
	return !before(tcp_sk(sk)->snd_una, newprague_ca(sk)->next_seq);
}

static u32 newprague_target_rtt(struct sock *sk)
{
	return newprague_ca(sk)->rtt_target;
}

static bool newprague_target_rtt_elapsed(struct sock *sk)
{
	return (newprague_target_rtt(sk) >> 3) <=
		tcp_stamp_us_delta(tcp_sk(sk)->tcp_mstamp,
				newprague_ca(sk)->alpha_stamp);
}
 ...
 * every RTT" */
static bool newprague_should_update_ewma(struct sock *sk)
{
	return newprague_e2e_rtt_elapsed(sk) &&
		(!newprague_is_rtt_indep(sk) ||
		 newprague_target_rtt_elapsed(sk));
}

static u64 newprague_unscaled_ai_ack_increase(struct sock *sk)
{
	return 1 << CWND_UNIT;
}

static u64 newprague_rate_scaled_ai_ack_increase(struct sock *sk, u32 rtt)
{
	u64 increase;
	u64 divisor;
	u64 target;

	target = newprague_target_rtt(sk);
	if (rtt >= target)
		return newprague_unscaled_ai_ack_increase(sk);
...
static u32 newprague_frac_cwnd_to_snd_cwnd(struct sock *sk)
{
	struct newprague *ca = newprague_ca(sk);
...
static u64 newprague_virtual_rtt(struct sock *sk)
{
	return max_t(u32, newprague_target_rtt(sk), tcp_sk(sk)->srtt_us);
}
...
static u64 newprague_pacing_rate_to_max_mtu(struct sock *sk)
{
	return div_u64(mul_64_64_shift(newprague_ca(sk)->rate_bytes, newprague_virtual_rtt(sk), 23) +
					(MIN_CWND_VIRT - 1), MIN_CWND_VIRT);
}

static bool newprague_half_virtual_rtt_elapsed(struct sock *sk)
{
	return (newprague_virtual_rtt(sk) >> (3 + 1)) <=
			tcp_stamp_us_delta(tcp_sk(sk)->tcp_mstamp,
			newprague_ca(sk)->alpha_stamp);
}

static u64 newprague_pacing_rate_to_frac_cwnd(struct sock *sk)
{
	struct newprague *ca = newprague_ca(sk);
	struct tcp_sock *tp = tcp_sk(sk);
	u64 rtt;
...
static u32 newprague_valid_mtu(struct sock *sk, u32 mtu)
{

	return max_t(u32, min_t(u32, newprague_ca(sk)->mtu_cache, mtu), tcp_mss_to_mtu(sk, MIN_MSS));
}

/* RTT independence will scale the classical 1/W per ACK increase. */
static void newprague_ai_ack_increase(struct sock *sk)
{
	struct newprague *ca = newprague_ca(sk);
	u64 increase;
	u32 rtt;
	...
	if (ca->round < ca->rtt_transition_delay ||
		...
		increase = newprague_unscaled_ai_ack_increase(sk);
		goto exit;
	}

	increase = newprague_rate_scaled_ai_ack_increase(sk, rtt);

exit:
	WRITE_ONCE(ca->ai_ack_increase, increase);
}

static void newprague_update_pacing_rate(struct sock *sk)
{
	struct newprague *ca = newprague_ca(sk);
	...
	u64 mtu;
	if (newprague_is_rtt_indep(sk)) {
		offset = mul_64_64_shift(ca->rate_offset, ca->rate_bytes, OFFSET_UNIT);
		if (newprague_half_virtual_rtt_elapsed(sk)) // second half
	...
	WRITE_ONCE(newprague_ca(sk)->max_tso_burst,
		   max_t(u32, 1, burst >> newprague_burst_shift));
	WRITE_ONCE(sk->sk_pacing_rate, rate);
}

static void newprague_new_round(struct sock *sk)
{
	struct newprague *ca = newprague_ca(sk);
	struct tcp_sock *tp = tcp_sk(sk);
	...
	newprague_ai_ack_increase(sk);
}

static void newprague_cwnd_changed(struct sock *sk)
{
	struct tcp_sock *tp = tcp_sk(sk);
...
static void newprague_update_alpha(struct sock *sk)
{
	struct newprague *ca = newprague_ca(sk);
	struct tcp_sock *tp = tcp_sk(sk);
	...
	if (ecn_segs) {
		...
		ecn_segs <<= NEWPRAGUE_ALPHA_BITS;
		ecn_segs = div_u64(ecn_segs, max(1U, acked_segs));
	}
	alpha = alpha - (alpha >> NEWPRAGUE_SHIFT_G) + ecn_segs;
	ca->alpha_stamp = tp->tcp_mstamp;
	alpha = min(NEWPRAGUE_MAX_ALPHA << NEWPRAGUE_SHIFT_G, alpha);

	WRITE_ONCE(ca->upscaled_alpha, alpha);
	tp->alpha = alpha >> NEWPRAGUE_SHIFT_G;

	if (newprague_is_rtt_indep(sk) && !ca->in_loss) {
		mtu_used = tcp_mss_to_mtu(sk, tp->mss_cache);
		mtu = newprague_valid_mtu(sk, newprague_pacing_rate_to_max_mtu(sk));
		if (mtu_used != mtu) {
			ca->frac_cwnd = div_u64(ca->frac_cwnd * mtu_used, mtu);
			tp->mss_cache_set_by_ca = true;
			tcp_sync_mss(sk, mtu);
			tp->snd_cwnd = newprague_frac_cwnd_to_snd_cwnd(sk);
		}
	}

	ca->hsrtt_us += tp->srtt_us - (ca->hsrtt_us >> HSRTT_SHIFT);
skip:
	newprague_new_round(sk);
}

static void newprague_update_cwnd(struct sock *sk, const struct rate_sample *rs)
{
	struct newprague *ca = newprague_ca(sk);
	struct tcp_sock *tp = tcp_sk(sk);
	u64 increase;
	...
	if (newprague_is_rtt_indep(sk)) {
		mtu_used = tcp_mss_to_mtu(sk, tp->mss_cache);
		increase = div_u64(((u64)(acked * MTU_SYS)) << 23, newprague_virtual_rtt(sk));
		divisor = mtu_used << 23;
		new_cwnd = div64_u64(ca->rate_bytes * newprague_virtual_rtt(sk) + divisor - 1, divisor);
		if (likely(new_cwnd))
			ca->rate_bytes += div_u64(increase + (new_cwnd >> 1), new_cwnd);
		ca->frac_cwnd = max_t(u64, ca->frac_cwnd + acked, newprague_pacing_rate_to_frac_cwnd(sk));
	} else {
...
adjust:
	new_cwnd = newprague_frac_cwnd_to_snd_cwnd(sk);
...
static void newprague_ca_open(struct sock *sk)
{
	newprague_ca(sk)->in_loss = 0;
}

static void newprague_enter_loss(struct sock *sk)
{
	struct newprague *ca = newprague_ca(sk);
	struct tcp_sock *tp = tcp_sk(sk);
	...
	ca->frac_cwnd = newprague_pacing_rate_to_frac_cwnd(sk);
	ca->in_loss = 1;
}

static void newprague_enter_cwr(struct sock *sk)
{
	struct newprague *ca = newprague_ca(sk);
	struct tcp_sock *tp = tcp_sk(sk);
	u64 reduction;
	...
	if (newprague_is_rtt_indep(sk)) {
		if ((newprague_target_rtt(sk) >> 3) > tcp_stamp_us_delta(tp->tcp_mstamp,
								       ca->cwr_stamp))
			return;
		ca->cwr_stamp = tp->tcp_mstamp;
		alpha = ca->upscaled_alpha >> NEWPRAGUE_SHIFT_G;

		reduction = mul_64_64_shift(ca->rate_bytes, alpha, NEWPRAGUE_ALPHA_BITS + 1);
...
static void newprague_state(struct sock *sk, u8 new_state)
{
	...
	case TCP_CA_Recovery:
		newprague_enter_loss(sk);
		break;
	case TCP_CA_CWR:
		newprague_enter_cwr(sk);
		break;
	case TCP_CA_Open:
		newprague_ca_open(sk);
		break;
	}
}

static void newprague_cwnd_event(struct sock *sk, enum tcp_ca_event ev)
{
	if (ev == CA_EVENT_LOSS)
		newprague_enter_loss(sk);
}

static u32 newprague_cwnd_undo(struct sock *sk)
{
	struct newprague *ca = newprague_ca(sk);
...
static void newprague_cong_control(struct sock *sk, const struct rate_sample *rs)
{
	newprague_update_cwnd(sk, rs);
	if (newprague_should_update_ewma(sk))
		newprague_update_alpha(sk);
	newprague_update_pacing_rate(sk);
}

static u32 newprague_ssthresh(struct sock *sk)
{
	struct tcp_sock *tp = tcp_sk(sk);
	return tp->snd_ssthresh;
}

static u32 newprague_tso_segs(struct sock *sk, unsigned int mss_now)
{
	u32 tso_segs = newprague_ca(sk)->max_tso_burst;

	if (newprague_max_tso_segs)
		tso_segs = min(tso_segs, newprague_max_tso_segs);

	return tso_segs;
}

static size_t newprague_get_info(struct sock *sk, u32 ext, int *attr,
			     union tcp_cc_info *info)
{
	const struct newprague *ca = newprague_ca(sk);

	if (ext & (1 << (INET_DIAG_PRAGUEINFO - 1)) ||
	    ext & (1 << (INET_DIAG_VEGASINFO - 1))) {
		memset(&info->prague, 0, sizeof(info->prague));
		if (inet_csk(sk)->icsk_ca_ops != &newprague_reno) {
			info->prague.prague_alpha =
				ca->upscaled_alpha >> NEWPRAGUE_SHIFT_G;
...
static void newprague_release(struct sock *sk)
{
	struct tcp_sock *tp = tcp_sk(sk);
...
static void newprague_init(struct sock *sk)
{
	struct newprague *ca = newprague_ca(sk);
	struct tcp_sock *tp = tcp_sk(sk);
	 ...
	 */
	ca->alpha_stamp = tp->tcp_mstamp;
	ca->upscaled_alpha = NEWPRAGUE_MAX_ALPHA << NEWPRAGUE_SHIFT_G;
	ca->frac_cwnd = ((u64)tp->snd_cwnd << CWND_UNIT);
	ca->max_tso_burst = 1;
	...
	ca->round = 0;
	ca->rtt_transition_delay = newprague_rtt_transition;
	ca->rtt_target = newprague_rtt_target << 3;
	...
	ca->rate_offset = (newprague_rate_offset && newprague_rate_offset < ((1 << OFFSET_UNIT) -1)) ? newprague_rate_offset : RATE_OFFSET ;

	tp->classic_ecn = 0ULL;
	tp->alpha = NEWPRAGUE_MAX_ALPHA;		/* Used ONLY to log alpha */

	newprague_new_round(sk);
}

static struct tcp_congestion_ops newprague __read_mostly = {
	.init		= newprague_init,
	.release	= newprague_release,
	.cong_control	= newprague_cong_control,
	.cwnd_event	= newprague_cwnd_event,
	.ssthresh	= newprague_ssthresh,
	.undo_cwnd	= newprague_cwnd_undo,
	.set_state	= newprague_state,
	.get_info	= newprague_get_info,
	.tso_segs	= newprague_tso_segs,
...
static struct tcp_congestion_ops newprague_reno __read_mostly = {
...
static int __init newprague_register(void)
{
	BUILD_BUG_ON(sizeof(struct newprague) > ICSK_CA_PRIV_SIZE);
	return tcp_register_congestion_control(&newprague);
}

static void __exit newprague_unregister(void)
{
	tcp_unregister_congestion_control(&newprague);
}

module_init(newprague_register);
module_exit(newprague_unregister);
...
MODULE_DESCRIPTION("TCP newPrague");
MODULE_VERSION("0.7");
